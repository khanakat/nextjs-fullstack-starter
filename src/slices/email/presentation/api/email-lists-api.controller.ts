/**
 * Email Lists API Controller
 * Handles HTTP requests for email list management
 */
...
@injectable()
export class EmailListsApiController {
  /**
   ...
   */
  async createEmailList(request: NextRequest): Promise<NextResponse> {
    const requestId = generateRequestId();
   ...
   */
  async getEmailLists(request: NextRequest): Promise<NextResponse> {
    const requestId = generateRequestId();
    let userId: string | null = null;
    let organizationId: string | null = null;
