...
export function useTemplates(filters: TemplateFilters = {}) {
  ...
  const createTemplate = async (templateData: Omit<Template, 'id' | 'createdAt' | 'updatedAt' | 'createdBy' | 'updatedBy' | 'usageCount'>) => {
    try {
  ...
  const toggleFavorite = async (id: string, isFavorited: boolean) => {
    try {
      ...
      if (!response.ok) {
        throw new Error(`Failed to ${isFavorited ? 'unfavorite' : 'favorite'} template`);
      }
      ...
      toast.success(`Template ${isFavorited ? 'removed from' : 'added to'} favorites`);
    } catch (err) {
