...
const COMPONENT_PALETTE = [
  ...
  },
  {
    ...
    icon: Calendar,
    label: "Filter",
    description: "Interactive filters",
  },
];
  ...
  return (
    ...
    >
      <Card className="shadow-sm">
        ...
        <CardContent className="pt-0">{renderComponentContent()}</CardContent>
      </Card>
    </div>
  );
}
...
export function ReportBuilder({ editReportId }: ReportBuilderProps) {
  ...
  const addComponent = useCallback((type: ComponentType) => {
    ...
    toast.success(
      `${type.charAt(0).toUpperCase() + type.slice(1)} component added`,
    );
  }, []);
  ...
  const updateComponentConfig = useCallback(
    (componentId: string, config: Partial<ReportComponent["config"]>) => {
      setComponents((prev) =>
        prev.map((c) =>
          c.id === componentId
  ...
  const updateComponentTitle = useCallback(
    (componentId: string, title: string) => {
      setComponents((prev) =>
        prev.map((c) => (c.id === componentId ? { ...c, title } : c)),
      );
    },
    [],
  );
        ...
        <ScrollArea className="flex-1 p-4">
          <div className="grid gap-3">
            {COMPONENT_PALETTE.map((item, index) => (
              <PaletteItem key={index} item={item} onDragStart={addComponent} />
            ))}
          </div>
        </ScrollArea>
      </div>
                  ...
                  {selectedComponentData.type === "chart" && (
                    <>
                      <div>
                        ...
                        <Select
                          value={
                            selectedComponentData.config.chartType || "bar"
                          }
                          onValueChange={(value) =>
                            updateComponentConfig(selectedComponentData.id, {
                ...
                <TabsContent value="style" className="space-y-4">
                      ...
                      <input
                        ...
                        onChange={(e) =>
                          updateComponentConfig(selectedComponentData.id, {
                            styling: {
                              width:
                                selectedComponentData.config.styling?.width ||
                                400,
                              height:
                                selectedComponentData.config.styling?.height ||
                                300,
                              ...selectedComponentData.config.styling,
                  ...
                  <div>
                    ...
                    <div className="flex items-center gap-2 mt-1">
                      <input
                        type="color"
                        value={
                          selectedComponentData.config.styling?.textColor ||
                          "#000000"
                        }
                        onChange={(e) =>
                          updateComponentConfig(selectedComponentData.id, {
                  ...
                  <div>
                    ...
                    <Input
                      type="number"
                      value={
                        selectedComponentData.config.styling?.fontSize || 14
                      }
                      onChange={(e) =>
                        updateComponentConfig(selectedComponentData.id, {
                          styling: {
