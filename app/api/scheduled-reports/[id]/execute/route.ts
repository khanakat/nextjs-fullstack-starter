 ...
 * Handles HTTP requests for executing scheduled reports using clean architecture
 */
...
export async function POST(request: NextRequest, { params }: RouteParams) {
  try {
    ...
    return await controller.executeScheduledReport(params.id, request);
  } catch (error) {
    console.error('Error in POST /api/scheduled-reports/[id]/execute:', error);
    return new Response(
