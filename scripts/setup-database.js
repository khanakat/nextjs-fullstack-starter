...
async function main() {
  ...
  console.log(`🏠 Local PostgreSQL: ${postgresLocalAvailable ? '✅ Available' : '❌ Not available'}`);
  console.log('');
  
  if (dockerAvailable) {
    console.log('🐳 Using Docker PostgreSQL...');
    updateEnvFile('postgresql://postgres:postgres123@localhost:5432/fullstack_template?schema=public', 'Docker PostgreSQL');
    ...
    updateEnvFile('postgresql://postgres:postgres@localhost:5432/fullstack_template?schema=public', 'Local PostgreSQL');
...
function fallbackToSQLite() {
  ...
  try {
    ...
    console.log('💡 To use PostgreSQL later, install Docker or PostgreSQL and run this script again.');
